...
use the helper function `createParameterList()`, which creates a list of 
...
The `seed` can be set for reproducibility and is straightforward.

The optional parameter `randomX1D` is a parameter that is relevant only with one-dimensional training data and determines whether the training data locations should be equally spaced or if they should be randomly sampled on $[0, 1]$. 
...
generated on a grid. For example, if `gridTest = FALSE`, then `nTest` test data locations are randomly selected on $[0, 1]^d$. However, if `gridTest = TRUE` and
